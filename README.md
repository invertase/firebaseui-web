...
```js
function FirebaseUIProvider({ uiCtx, children }) {
  const config = useContext(uiCtx); // Set your context state here.

  return <YourContext.Provider value={config}>{children}</YourContext.Provider>;
}
```
